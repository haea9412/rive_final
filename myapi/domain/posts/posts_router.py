...
def post_update(_post_update: posts_schema.PostUpdate, db: Session = Depends(get_db), current_user: Users = Depends(get_current_user)):
    db_post = posts_crud.get_post(db, post_id=_post_update.post_id)
    if not db_post:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail="데이터를 찾을 수 없습니다.")
    if current_user.id != db_post.users.id:
        raise HTTPException(status_code=status.HTTPS_400_BAD_REQUEST,
                            detail="수정 권한이 없습니다.")
